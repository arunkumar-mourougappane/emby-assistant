# Documentation

This folder contains all documentation for the Emby Assistant project.

## 📑 Available Documents

| Document                           | Description                                  |
|------------------------------------|----------------------------------------------|
| **[INDEX.md](INDEX.md)**           | Complete documentation index with navigation |
| **[QUICKSTART.md](QUICKSTART.md)** | Quick start guide for fast setup             |
| **[README-GTK.md](README-GTK.md)** | GTK desktop version documentation            |
| **[COMPARISON.md](COMPARISON.md)** | Web vs GTK version comparison                |
| **[THUMBNAILS.md](THUMBNAILS.md)** | Thumbnail feature documentation              |
| **[PROJECT.md](PROJECT.md)**       | Project overview and structure               |

## 🚀 Where to Start?

- **New users**: Start with [../README.md](../README.md) then [QUICKSTART.md](QUICKSTART.md)
- **GTK users**: Read [README-GTK.md](README-GTK.md)
- **Choosing version**: See [COMPARISON.md](COMPARISON.md)
- **Full navigation**: Browse [INDEX.md](INDEX.md)

## 📖 Main Documentation

The main project documentation is in [../README.md](../README.md)
